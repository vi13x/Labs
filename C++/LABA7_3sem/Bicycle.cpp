...
{
    if (Gears < 1)
        throw InvalidArgumentException("gears", "Количество передач должно быть не менее 1");
    
    if (Gears > 30)
        throw OverflowErrorException("установка количества передач", "Количество передач превышает максимальное значение (30)");
...
bool Bicycle::can_climb_hill(double gradient) const
{
    if (std::isinf(gradient) || std::isnan(gradient))
        throw DomainErrorException("Градиент подъема не может быть бесконечным или NaN");
    
    if (gradient < 0)
        throw InvalidArgumentException("gradient", "Градиент подъема не может быть отрицательным");
...
{
    if (gears == 0)
        throw DomainErrorException("Невозможно вычислить коэффициент усилия: количество передач равно 0");
    
    double result = 10.0 / gears;
    
    if (std::isinf(result) || std::isnan(result))
        throw OverflowErrorException("вычисление коэффициента усилия", "Результат переполнения");
